...
export async function POST() {
  try {
    console.log('🚀 [FORCE-POPULATE] Starting forced population of Sanity backend...')
    ...
    let result
    if (existingSettings) {
      console.log('📝 [FORCE-POPULATE] Updating existing settings document...')
      result = await writeClient
        ...
        .set({
          ...
          title: existingSettings.title || defaultData.title,
          subtitle: existingSettings.subtitle || defaultData.subtitle,
          templateDownloadText: existingSettings.templateDownloadText || defaultData.templateDownloadText,
          submissionEnabled: existingSettings.submissionEnabled !== undefined ? existingSettings.submissionEnabled : defaultData.submissionEnabled
        })
        .commit()
      
      console.log('✅ [FORCE-POPULATE] Successfully updated existing document:', result._id)
    } else {
      console.log('📄 [FORCE-POPULATE] Creating new settings document...')
      result = await writeClient.create({
        _type: 'abstractSettings',
        ...defaultData
      })
      
      console.log('✅ [FORCE-POPULATE] Successfully created new document:', result._id)
    }
    ...
    return NextResponse.json({
      ...
      interestedInOptionsCount: defaultData.interestedInOptions.length,
      trackNamesCount: defaultData.trackNames.length,
      data: {
        ...defaultData,
        _id: result._id
      }
    ...
    console.error('❌ [FORCE-POPULATE] Error populating Sanity:', error)
    return NextResponse.json({
      success: false,
      error: error.message,
